...
import {NumberKey} from "../types/Intermediate";
...
export interface Context {
	...
	/**
	 * Creates an {@link InsightError} with the current path and the reason for failiure.
	 *
	 * @param reason The reason the parsing failed
	 * @returns An error instance
	 */
	failure(reason: string): InsightError;
}
...
export const context = (path: Path = [], shared: SharedContext = sharedContext()): Context => ({
	...
	failure: (reason) => new InsightError(`Parsing query failed at ${formatPath(path)}: ${reason}`),
});
