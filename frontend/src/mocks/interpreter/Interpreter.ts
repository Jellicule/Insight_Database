 ...
 */
export function evalQuery(query: Query, dataset: ReadonlyArray<SectionInternal | RoomInternal>): PartialResult[] {
...
function evalStandardQuery(query: StandardQuery, records: readonly PartialResult[]): PartialResult[] {
	...
	return sorted.map((record) => pick(record, ...columns.map((k) => k.value))) as PartialResult[];
}

const prefixKeys = <T>(object: Record<string, T>, prefix: string) =>
	Object.fromEntries(Object.entries(object).map(([k, v]) => [`${prefix}_${k}`, v]));

function evalAggregateQuery(query: AggregateQuery, records: readonly PartialResult[]): PartialResult[] {
	...
	return selected as PartialResult[];
}
...
function groupByWithLimit<T, K>(
	...
	for (const element of array) {
		...
		if (groups.size > limit) {
			throw new Error("Too many groups in aggregation query result.");
		}
	}
...
function sortByOrder(order: Order | undefined, sections: readonly PartialResult[]): readonly PartialResult[] {
	const cloned = [...sections] as PartialResult[];
	if (order !== undefined) {
...
function filterWithLimit<T>(
	...
	return array.filter((value, index) => {
		...
		if (count > limit) {
			throw new Error("Too many records in query result.");
		}
		return result;
	});
}
